...
# ✅ دالة الحذف (الجديدة)
def delete_from_firebase(nom_fournisseur):
    try:
        if nom_fournisseur:
            doc_id = str(nom_fournisseur).lower().strip().replace("/", "_")
            db.collection("suppliers").document(doc_id).delete()
    except Exception as e:
    ...
    with st.form("manual_form"):
        ...
        if st.form_submit_button("💾 Enregistrer"):
            if name:
                ...
                st.error("Veuillez entrer le nom")
...
df = df[df.apply(lambda row: not row.astype(str).str.lower().str.contains("enterprise test").any(), axis=1)]
    ...
    # ✅ واجهة الحذف
    st.markdown("### 🗑️ Supprimer un fournisseur")
    nom_to_delete = st.selectbox(
        "Choisir le fournisseur à supprimer",
        options=[item['Nom du Fournisseur'] for item in st.session_state.data_list]
    )

    if st.button("❌ Supprimer"):
        delete_from_firebase(nom_to_delete)
        st.session_state.data_list = load_from_firebase()
